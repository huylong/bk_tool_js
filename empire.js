...
const GAME_SETTINGS = {
    minDelay: 300,
    maxDelay: 500,
    minPauseDuration: 300000,
    maxPauseDuration: 480000,
    reloadDelay: 3000
};
...
settingsMenu.appendChild(createSettingElement('Độ trễ tối thiểu (ms)', 'minDelay', 'range', 10, 1000, 10,
    'EN: Độ trễ tối thiểu giữa các lần nhấp.'));
settingsMenu.appendChild(createSettingElement('Độ trễ tối đa (ms)', 'maxDelay', 'range', 10, 1000, 10,
    'EN: Độ trễ tối đa giữa các lần nhấp.'));
settingsMenu.appendChild(createSettingElement('Thời gian tạm dừng tối thiểu (ms)', 'minPauseDuration', 'range', 10000, 300000, 1000,
    'EN: Thời gian tạm dừng tối thiểu khi năng lượng thấp.'));
settingsMenu.appendChild(createSettingElement('Thời gian tạm dừng tối đa (ms)', 'maxPauseDuration', 'range', 10000, 300000, 1000,
    'EN: Thời gian tạm dừng tối đa khi năng lượng thấp.'));
...
function toggleAutoclicker() {
    ...
    updateInfoElement(isAutoclickerPaused ? "Autoclicker đã tạm dừng" : "Autoclicker đang chạy");
}
...
function startPauseTimer(duration) {
    ...
    const updateTimer = () => {
        updateInfoElement(`Tạm dừng: còn lại ${remainingTime} giây`, 0);
        remainingTime--;
        if (remainingTime >= 0) {
            setTimeout(updateTimer, 1000);
        } else {
            updateInfoElement("Autoclicker đang tiếp tục...");
        }
    };
    updateTimer();
}
...
function parseEnergy(energyText) {
    energyText = energyText.replace(/\s/g, '').replace(/\./g, '');
...
function autoclicker() {
    ...
    if (telegramImg) {
        console.log("Phát hiện mã QR của Telegram. Trang sẽ được tải lại sau 3 giây.");
        updateInfoElement("Tải lại trang trong 3 giây...");
        setTimeout(() => {
            location.reload();
    ...
    if (!energyEl) {
        console.log("Không tìm thấy phần tử năng lượng. Thử lại sau 1 giây.");
        updateInfoElement("Đang tìm kiếm phần tử năng lượng...");
    ...
    if (energy <= 500) {
        ...
        console.log(`Năng lượng thấp (${energy}), tạm dừng trong ${pauseSeconds} giây.`);
    ...
    if (checkTooManyTaps()) {
        const pauseDuration = getRandomDelay(60000, 70000);
        const pauseSeconds = Math.floor(pauseDuration / 1000);
        console.log(`Đã phát hiện quá nhiều lần nhấn, tạm dừng trong ${pauseSeconds} giây.`, 0);
        startPauseTimer(pauseDuration);
        setTimeout(autoclicker, pauseDuration);
        return;
    }

    updateInfoElement(`Autoclicker đang hoạt động. Năng lượng: ${energy}`, 0);


    setTimeout(autoclicker, getRandomDelay(GAME_SETTINGS.minDelay, GAME_SETTINGS.maxDelay));
}

function checkTooManyTaps() {
    var textElement = document.querySelector("body > div.van-popup.van-popup--top.van-notify.van-notify--danger");

    if (textElement && textElement.textContent.includes("too many taps")) {
        const style = window.getComputedStyle(textElement);
        if (style.display === 'block') {
            return true; // Phần tử đang hiển thị và chứa thông điệp
        }
    }

    return false; // Không tìm thấy hoặc không hiển thị
}
...
function initializeScript() {
    loadSettings();
    updateSettingsMenu();
    updateInfoElement("Autoclicker đang được khởi động...");
